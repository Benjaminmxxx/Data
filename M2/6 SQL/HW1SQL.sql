...
SELECT * FROM carrera; -- Comprobación de que se han insertado correctamente los datos
...
SELECT * FROM instructor i ; -- Comprobación
...
SELECT * FROM cohorte c; #Comprobacion
...
SELECT * FROM alumno; #COMPROBACIÓN
...
VALUES (161,'41944781','Stephanie','Hurst','1986-11-23','2022-02-14',1243),
 ...
 SELECT * FROM alumno a; #COMPROBANDO
...
#2.	No se sabe con certeza el lanzamiento de las cohortes N° 1245 y N° 1246, se solicita que las elimine de la tabla.
 ...
 WHERE 
idCohorte IN (1245,1246); #CONSULTANDO

 DELETE FROM cohorte WHERE idCohorte IN (1245,1246); #ELIMINANDO
 
 #CONSULTAR NUEVAMENTE (CORRECTO)
...
UPDATE cohorte 
SET fechaInicio = '2022-05-16' #LAS DATES SE ESCRIBEN COMO UN STRING (YYYY/MM/DD) 
WHERE idCohorte = 1243;
...
SELECT nombre,fechaIngreso  FROM
alumno a WHERE idCohorte =1243;
...
SELECT * FROM cohorte c; #VER LA TABLA
...
#9. La siguiente consulta permite acceder a datos de otras tablas y devolver un listado con la carrera 
#a la cual pertenece cada alumno:

SELECT * FROM alumno a;
SELECT * FROM cohorte c;
SELECT * FROM carrera c;

SELECT a.idAlumno ,a.nombre,a.apellido, ca.nombre as Nombre_Carrera
FROM alumno a
JOIN cohorte c ON c.idCohorte = a.idCohorte 
JOIN carrera ca ON ca.idCarrera = c.idCarrera;

#10. De la siguiente Query:

SELECT alumno.nombre, apellido, fechaNacimiento, carrera.nombre 
FROM alumno
INNER JOIN cohorte ON cohorte.idCohorte = alumno.idCohorte
INNER JOIN carrera ON carrera.idCarrera = cohorte.idCarrera;

# a. ¿Que campos permiten que se puedan acceder al nombre de la carrera?

-- Los INNER JOIN son los que permiten el acceso a los datos de la tabla carrera

# b. ¿Que tipo de ralación existe entre el nombre, la tabla cohortes y alumnos?

-- La primer relación es "idCarrera" de carrera y cohorte, despues entre coherte y alumno "idCohorte"

# c. ¿Proponga dos opciones para filtrar el listado solo por los alumnos que pertenecen 
# a la carrera de 'Full Stack Developer', utilizando LIKE y NOT LIKE?, 
# ¿Cual de las dos opciones es la manera correcta de hacerlo?, ¿Por que?

-- OPCION 1: --ESTA OPCION ES MAS ACERTADA YA QUE CON NOT LIKE ES NECESARIO SABER QUE OTRAS CARRERAS EXISTEN 
...
-- OPCION 1 #ESTA ME PARECE MAS CORRECTA TOMANDO EL RAZONAMIENTO ANTERIOR
SELECT alumno.nombre, apellido, fechaNacimiento, carrera.nombre 
FROM alumno
INNER JOIN cohorte ON cohorte.idCohorte = alumno.idCohorte
INNER JOIN carrera ON carrera.idCarrera = cohorte.idCarrera
